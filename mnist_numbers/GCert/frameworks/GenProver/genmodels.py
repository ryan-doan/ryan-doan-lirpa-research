...
def FFNN(layers, last_lin = False, last_zono = False, **kargs):
    ...
    if last_lin:
        ends = (
            [n.CorrelateAll(only_train=False)] if last_zono else []
        ) + [
                n.PrintActivation(activation = "Affine"),
                n.Linear(layers[-1], **kargs)
        ]
...
def InvLeNet(ly, w, conv_layers, bias=True, normal=False, **kargs):
    def transfer(tp, lin = False):
        return (n.ConvTranspose2D if lin else n.ConvTranspose)(
                                        out_channels=tp[0],
                                        kernel_size=tp[1],
                                        stride=tp[2],
                                        padding=tp[3],
