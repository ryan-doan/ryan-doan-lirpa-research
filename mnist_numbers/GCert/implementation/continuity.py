...
def continuity(generator, z, device, uniform = False):
    ...
    z1 = torch.randn(z.shape, device=device)
    z2 = torch.randn(z.shape, device=device)
    if uniform:
        z1 = torch.rand(z.shape, device=device)
        z2 = torch.rand(z.shape, device=device)
    G1 = generator(z1)
    G2 = generator(z2)
